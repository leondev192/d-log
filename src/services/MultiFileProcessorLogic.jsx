...
// Constants
const GOOGLE_VISION_API_KEY = import.meta.env.VITE_GOOGLE_VISION_API_KEY;
const GEMINI_API_KEY = "AIzaSyAt5g2wh2QDVEK7iL8G34YlQiNXgo-EBgI";
// console.log(GEMINI_API_KEY, GOOGLE_VISION_API_KEY);
const GEMINI_API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${GEMINI_API_KEY}`;
...
2. H-Bill of Lading No.:(Extract only the house bill of lading number from the contain of which file contains the information "HOUSE BILL OF LADING"): …(example HCMBKK029112022)
3. Description of Goods (extract only the description): …(example FABRIC 100 PCT. POLYESTER W: 114 CM)
4. Consignor/Shipper (include only company name and address, separated by "#"): …(example SUGATTI FREIGHT INT'L (THAILAND) CO., LTD # 11/38 SUKHUMVIT ROAD FRAXANONG, KLONGTOEY)
5. Consignee/Consigned to Order of (include only tax ID, company name, and address, separated by "#"): …(example 0301972094 # VIET HOA TRANSPORT SERVICE AND TRADING CO., LTD. # 284 NGUYEN TAT THANH STR, WARD 13, DIST 4, HCMC, VIETNAM) 
6. Notify Party : …(example SAME AS ABOVE)
7. Port of Loading (convert to port code using the table below if matched): …(example VNCLI)
8. Port of Discharge (convert to port code using the table below if matched): …(example THBKK)
9. Đến cảng (Extract only the port name from the contain of which file contains the information "Arrival Notice"): …(example CFS TNHH NGỌC KHANH)
10. Number of Packages (extract only the quantity): …(example 1)
11. Kind of Packages (convert to code using the table below if matched): …(example CT)
12. Container No. (Extract only the container number from the contain of which file contains the information "Consignor” or “Shipper"): …(example CSLU2082865)
13. Seal No. (Extract only the seal number from the contain of which file contains the information "Consignor” or “Shipper"):  …(example 21567932)
